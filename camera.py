...
def box_draw(image_list, bbox_list, rg_net, rg_ln, labels, dt_net, dt_ln, lp_confss, 
             obj_id=None, best_global_frame=None, best_plate_bbox=None, camera_name=None, config=None):
    ...
    if consolidated:
        ...
        if processor:
            ...
            t = Thread(target=text_process_context_wrapper, args=(lp_image, consolidated, group_summary, lp_confss, full_frame, obj_id, best_plate_bbox))
            t.start()
...
def ocr_worker(rg_net, rg_ln, labels, dt_net, dt_ln, config):
    ...
    while True:
        try:
            ...
            obj_id, img_list, bbox_list, lp_confss, best_frame, best_plate_bbox, is_final, cam_name = data
            ...
            box_draw(img_list, bbox_list, rg_net, rg_ln, labels, dt_net, dt_ln, lp_confss, 
                     obj_id, best_frame, best_plate_bbox, cam_name, config)
...
def offload_tracks_to_queue(completed_tracks, camera_name, config, zone_polygons=None):
    ...
    for entry in completed_tracks:
        obj_id, img_list, bbox_list, lp_confss, zone_history, is_final, best_full, best_plate_bbox, displacement_vector = entry
        ...
        if dir_cfg.get("enabled"):
            ...
            if clean_seq:
                ...
                if not is_valid and zone_polygons and "A" in zone_polygons and "B" in zone_polygons:
                    ...
                    dist = (displacement_vector[0]**2 + displacement_vector[1]**2)**0.5
                    dot_prod = displacement_vector[0] * ref_vec[0] + displacement_vector[1] * ref_vec[1]
                    
                    # Check if movement aligns with reference and exceeds minimum thresholds
                    if len(img_list) >= min_det and dist >= min_disp and dot_prod > 0:
                        is_valid = True
                        log.info(f"[Direction] ID:{obj_id} Validated by movement (Dist:{dist:.1f}, Dot:{dot_prod:.1f})")
                
                if not is_valid:
                    log.info(f"[Direction] ID:{obj_id} Rejected {mode} path:{clean_seq} disp:{displacement_vector}")

        if is_valid and not ocr_queue.full():
            ocr_queue.put((obj_id, img_list, bbox_list, lp_confss, best_full, best_plate_bbox, is_final, camera_name))
...
def camera_main():
    ...
    det_model, ocr_model, vehicle_model = container.load_models(
