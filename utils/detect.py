    ...
    def __new__(cls):
        with cls._lock:
            if cls._instance is None:
                ...
                cls._instance.vehicle_model = None
        return cls._instance

    def load_models(self, det_weight, ocr_weight, vehicle_weight=None, device="auto", use_fp16=True):
        ...
        with self._lock:
            ...
            # Load Vehicle Detection
            if vehicle_weight and self.vehicle_model is None:
                logging.info(f"Loading Vehicle Model: {vehicle_weight} (FP16={use_fp16})")
                self.vehicle_model = YOLO(vehicle_weight)
                self.vehicle_model.to(resolved_device)
                if use_fp16 and resolved_device == "cuda":
                    self.vehicle_model.half()
                logging.info("Vehicle Detection Model loaded and shared.")
        return self.detection_model, self.ocr_model, self.vehicle_model
...
def get_vehicle_type(image, plate_bbox, model, threshold=0.3):
    """
    Identifies vehicle type (car, motorcycle, bus, truck) and returns
    its bounding box [x1, y1, x2, y2].
    """
    if model is None or image is None:
        return "car", None # Default fallback

    # 1. Run Vehicle Detection
    is_half = next(model.model.parameters()).dtype == torch.float16
    results = model(image, conf=threshold, verbose=False, half=is_half)[0]
    
    # COCO Category Mapping for YOLOv8
    # 2: car, 3: motorcycle, 5: bus, 7: truck
    category_map = {2: "car", 3: "motorcycle", 5: "bus", 7: "truck"}
    
    plate_x1, plate_y1 = plate_bbox[0]
    plate_x2, plate_y2 = plate_bbox[1]
    plate_cx, plate_cy = (plate_x1 + plate_x2) / 2, (plate_y1 + plate_y2) / 2
    ...
    for box in results.boxes:
        ...
        if vx1 <= plate_cx <= vx2 and vy1 <= plate_cy <= vy2:
            return category_map[cls_id], (int(vx1), int(vy1), int(vx2), int(vy2))
